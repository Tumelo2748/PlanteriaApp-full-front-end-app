...
const ModelScreen = ({ route, navigation }) => {
    const { plantName, plantDate, plantImage, plantData } = route.params;
    const navigateBack = () => {
        navigation.goBack(); // Navigate back to the previous screen (Information screen)
    };

    const formattedDate = plantDate ? plantDate.toDate().toLocaleString() : 'No date available';
    ...
    return (
        <View style={{ flex: 1, marginTop: 40 }}>
            ...
            <View style={myStyles.Container}>
                ...
                <Text style={myStyles.header}>Date: {formattedDate}</Text>
